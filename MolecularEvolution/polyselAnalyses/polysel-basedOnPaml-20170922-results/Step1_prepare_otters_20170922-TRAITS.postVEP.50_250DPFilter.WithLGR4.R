...
# 1. gene2ensembl: List of gene UIDs from NCBI and their corresponding ensembl IDs
gene2ensembl <- read.table("inputFormatting/20170525/gene2ensembl",comment.char = "",header=T)
dim(gene2ensembl) # 650676      7
head(gene2ensembl)

# 2. biosystems_gene_all: Set of all biosystems and genes that are in them. Note that conserved biosystems include gene IDs from all species they know about (so could have dog KRT4, human KRT4, danio KRT4, etc. This is good)
# only need to do this once, is slow.
...
# osmoregulation, thermoregulation, hair follicle, diet, sensory perception, hypoxia
...
#rep="1"
pamlDate="20170922"
## read in the file that has been through Step 8 in R already. (has symbols, etc. )
...
dim(codeml) # 11083
# want to exclude sites with BEB > 1 (outliers)
...
# so merge again just on that and combine? that might work
head(codeml)
...
datadir=paste("data/otters/paml.",pamlDate,"-traits-postVEP_50_250DPFilter-WithLGR4/",branch,sep="")
...
#setInfo <- conserved_bsid_names[,c("BSID","Name","Source")]
...
# for now only using conserved sets # accessed 20170525
# download as csv from ncbi biosystems (search type = pathway, then select conserved)

#### Of. make SETOBJ file with BSID and gene ID (only conserved biosystems) #### 
head(conserved_bsid)
#biosystems_gene_conserved <- biosystems_gene_all[(biosystems_gene_all$bsid %in% conserved_bsid$bsid),]
...
dim(conserved_bsid) # 32990
length(unique(biosystems_gene_conserved$bsid)) #
# the ones that are missing don't have any genes associated with them (I checked a few)
head(biosystems_gene_conserved)
