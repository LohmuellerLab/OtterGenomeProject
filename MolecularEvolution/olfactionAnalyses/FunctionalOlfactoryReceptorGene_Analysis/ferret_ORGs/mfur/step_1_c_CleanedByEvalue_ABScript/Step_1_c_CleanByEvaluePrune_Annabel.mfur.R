...
scaffLengths <- read.table("Mustela_putorius_furo.MusPutFur1.0.dna.toplevel.fa.fai",header=F)
...
bed
dim(bed) # 716
write.table(bed,"step_1_c_CleanedByEvalue_ABScript/mfur.cleanedByEvalue.LengthFilterOnly.Pruned.scaff.0based.start.minus300.stop.plus300.name.eval.strand.bed",row.names = F,col.names = F,quote=F,sep="\t")
