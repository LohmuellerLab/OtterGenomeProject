...
############ SET UP GROUPS ###########
# outgroup
outgroup <- tree$tip.label[grep("mfur",tree$tip.label,invert=T)]
outgroup <- outgroup[outgroup!="Human_OR2J3"]
outgroup <- outgroup[grep("Clade",outgroup,invert=T)]
mfur <- tree$tip.label[grep("mfur",tree$tip.label,invert=F)]
# human
...
# shorten labels? rememver geom_label_repel for later
p
outlier <- "mfur_GL897417.1_82828_83739_1-323_aa" # visual inspection. Remove this sequence
outlierNode=1679
...
classITips_sppOnly <- classITips[grep("mfur",classITips)] 
length(classITips_sppOnly) # (77 in elut) --> 141 in mfur (incomplete genome or pseudogenes?)
# class II;  
classIInode  = 1677
viewClade(ggtree(tree)+geom_tiplab(), node=classIInode)
classIITips <- tips(tree,node=classIInode)
classIITips_sppOnly <- classIITips[grep("mfur",classIITips)] # (elut: 398), mfur: 682
...
###### Plot positions (? how am I going to get final sequence positions? blast? local realignment? based on where the M is?) #########
