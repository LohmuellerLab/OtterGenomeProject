...
scaffLengths <- read.table("PteBra.a.lines.fasta.fai",header=F)
...
bed
dim(bed) # 716
write.table(bed,"step_1_c_CleanedByEvalue_ABScript/pbra.cleanedByEvalue.LengthFilterOnly.Pruned.scaff.0based.start.minus300.stop.plus300.name.eval.strand.bed",row.names = F,col.names = F,quote=F,sep="\t")
...
write.table(pullOut,"step_1_c_CleanedByEvalue_ABScript/pbra.pullout.LengthFilter250.allInfo.txt",row.names = F,quote=F,sep = "\t")
