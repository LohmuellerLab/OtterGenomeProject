...
############# DON'T REMOVE >250 AAS FOR PSEUDOGENE ANALYSIS
blastsumDD <- blastsumDD_raw
dim(blastsumDD) # 411438 (without < 250AAs : 318573)
...
# check it! 
check <- findOverlaps(keep_prune_eval,maxgap=100,minoverlap=1,drop.self=F,drop.redundant=F) # 1228 (855 once filtered)
...
bed
dim(bed) # 716
write.table(bed,"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/elut2.cleanedByEvalue.NoLengthFilter.Pruned.scaff.0based.start.minus300.stop.plus300.name.eval.strand.bed",row.names = F,col.names = F,quote=F,sep="\t")

#write.table(pullOut,"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/elut2.cleanedByEvalue.NoLengthFilter.Pruned.all.pullOut.info.txt",row.names = F,col.names = T,quote=F,sep="\t")
#### EXCLUDE FUNCTIONAL ORS  ####
pullOut <- read.table("step_1_c_CleanedByEvalue_ABScript_noLengthFilter/elut2.cleanedByEvalue.noLengthFilter.Pruned.all.pullOut.info.txt",sep="\t",header=T)
dim(pullOut) # 1228
head(pullOut)
...
View(pullOut)
# missing: "elut2_KZ291759.1_41704852_41705703_-1"
# find neighbor 
...
dim(nonFunctionalORs_bed)
write.table(nonFunctionalORs_bed,"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/nonFunctionalORs.elut2.bed",quote=F,row.names=F,col.names=F,sep="\t")
############################## Separate Clade Info ################
...
write.table(pullOut,"step_1_c_CleanedByEvalue_ABScript/pullOut.mfur.fullInfo.Lengthrestriction250.txt",row.names=F,sep="\t",quote=F)
...
write.table(c(as.character(closeToStartOFScaffs),as.character(closeToEndOFScaffs)),"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/regionsCloseToStartEndofScaffold.elut2.txt",row.names=F,col.names=F,quote=F)
################### Plot #################################
require(ggplot2)
