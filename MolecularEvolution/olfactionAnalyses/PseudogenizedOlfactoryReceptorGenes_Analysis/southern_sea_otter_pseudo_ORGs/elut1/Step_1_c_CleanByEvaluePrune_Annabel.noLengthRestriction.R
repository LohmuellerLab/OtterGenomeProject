...
blastsum <- read.table("olfac_5sp_2seq_query.NOSPACES.sum",sep="/",header=T)
dim(blastsum)
head(blastsum)
...
# dim(dupScaffs_99)
 blastsum_nodup <- blastsum[!(blastsum$Hit %in% dupScaffs_99$V1),]
write.table(blastsum_nodup,"/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/20170707_MainOlfactionResults/olfac_5sp_2seq_query.NOSPACES.NoDupScaffs99.sum",row.names=F,sep="/",quote=F) # wrote this out on 20170921
# dim(blastsum_nodup) # 358991
# dim(blastsum)

########### read back in the deduped (DD) results that you just wrote out (once) #########
blastsumDD_raw <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/20170707_MainOlfactionResults/elut1/step_1_ab_blastDb_blastResults/olfac_5sp_2seq_query.NOSPACES.NoDupScaffs99.sum",header=T,sep="/")
# do some filtering ahead of time to speed up processing:

dim(blastsumDD_raw) # 358991 
...
dim(blastsumDD) # 358991 (217599 when removing < 250 AAs)
