...
################# NON FUNCTIONAL ###############
################## a. TRUNCATED ###############
truncORs_NC <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/elut1/elut1.dup/step_4_b_visualize/VisualizationNotes/elut1.dup.Nmissing.Cmissing.IDList.format.txt",header=T,strip.white=T) # this is list of truncated ORs. 
dim(truncORs_NC)
truncORs_N <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/elut1/elut1.dup/step_4_b_visualize/VisualizationNotes/elut1.dup.Nmissing.Ccomplete.IDList.format.txt",header=T,strip.white=T)
dim(truncORs_N)
truncORs_C <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/elut1/elut1.dup/step_4_b_visualize/VisualizationNotes/elut1.dup.Ncomplete.Cmissing.IDList.format.txt",header=T,strip.white=T)
dim(truncORs_C)
notTrunc <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/elut1/elut1.dup/step_4_b_visualize/VisualizationNotes/elut1.dup.Ncomplete.Ccomplete.IDList.format.txt",header=T,strip.white = T)
dim(notTrunc)
...
# check:
sum(pullOut_all$label=="pseudogene") # 27
sum(pullOut_all$label=="functional") #  61
sum(pullOut_all$label=="truncated") # 71
sum(pullOut_all$label=="fragment") # 45
# cool. 
#write.table(pullOut_all,"step_5_labelAllgenes/pullOut.func.trunc.pgene.labelled.elut1.dup.all.fragments.txt",row.names=F,quote = F,sep="\t")
################# PLOT GENES BY CLADE ################
...
p <- ggplot(pullOut_all,aes(x=clade,fill=label))+
  geom_bar(stat="count")+
  theme_bw()+
  scale_fill_manual(values=c(func,pg,tr,"darkgrey"))+
