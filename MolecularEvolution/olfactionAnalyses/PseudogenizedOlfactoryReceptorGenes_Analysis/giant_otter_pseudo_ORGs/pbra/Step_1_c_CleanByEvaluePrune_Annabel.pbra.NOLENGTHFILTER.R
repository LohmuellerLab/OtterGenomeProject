...
setwd("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/pbra")

########### read back in the deduped (DD) results that you just wrote out (once) #########
blastsumDD_raw <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/20170707_MainOlfactionResults/pbra/step_1_ab_blastDb_blastResults/olfac_5sp_2seq_query.pbra.NOSPACES.sum",header=T,sep="/",quote="")
# do some filtering ahead of time to speed up processing:
# get rids of hits < 250bp in length
dim(blastsumDD_raw) # 444985 
...
# DONT DO LENGTH RESTRICTION FOR PGENE ANAYSIS
blastsumDD <- blastsumDD_raw
dim(blastsumDD) # 444985 (318573 WHEN LENGTH RESTRICTED)
...
length(overlaps) # withonly length filter: 90060732
...
scaffLengths <- read.table("../../20170707_MainOlfactionResults/pbra/PteBra.a.lines.fasta.fai",header=F)
...
pullOut$bedname <- paste("pbra",pullOut$Hit,pullOut$Hit_Start,pullOut$Hit_END,pullOut$Hit_strand,sep="_")
bed <- pullOut[,c("Hit","bed_HitStart0_minus300","bed_HitEnd_plus300","bedname","E.value","bedstrand")]
bed
dim(bed) # 716
write.table(bed,"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/pbra.cleanedByEvalue.LengthFilterOnly.Pruned.scaff.0based.start.minus300.stop.plus300.name.eval.strand.bed",row.names = F,col.names = F,quote=F,sep="\t")
write.table(pullOut,"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/pbra.cleanedByEvalue.noLengthFilter.Pruned.all.pullOut.info.txt",row.names = F,col.names = T,quote=F,sep="\t")
#### EXCLUDE FUNCTIONAL ORS  ####
pullOut <- read.table("step_1_c_CleanedByEvalue_ABScript_noLengthFilter/pbra.cleanedByEvalue.noLengthFilter.Pruned.all.pullOut.info.txt",sep="\t",header=T)
dim(pullOut) # 1439 
head(pullOut)
functionalORs <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/20170707_MainOlfactionResults/pbra/step_9_FinalFunctionalList/classI.and.classII.final.ORs.passedInspection.txt",header=F) # this is the result of step 9 of functional analysis
dim(functionalORs)  # 585
...
missing <- functionalORs_names[!(functionalORs_names %in% pullOut$bedname)] # missing 
...
write.table(nonFunctionalORs_bed,"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/nonFunctionalORs.pbra.bed",quote=F,row.names=F,col.names=F,sep="\t")
############################## Separate Clade Info ################
...
write.table(c(as.character(closeToStartOFScaffs),as.character(closeToEndOFScaffs)),"step_1_c_CleanedByEvalue_ABScript_noLengthFilter/regionsCloseToStartEndofScaffold.pbra.txt",row.names=F,col.names=F,quote=F)
################### Plot #################################
require(ggplot2)
