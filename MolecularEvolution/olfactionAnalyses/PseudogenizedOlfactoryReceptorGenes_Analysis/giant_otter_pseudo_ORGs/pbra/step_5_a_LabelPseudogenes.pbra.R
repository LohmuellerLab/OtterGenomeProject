...
pullOut_LengthFilter <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/20170707_MainOlfactionResults/pbra/step_1_c_CleanedByEvalue_ABScript/pbra.pullout.LengthFilter250.allInfo.txt",sep="\t",header=T) # these don't include too short genes, but include the best long hit that you want to use. Some best long hits got swapped for shorterhits with higher evalues. need to deal with that 
pullOut_noLF <- read.table("step_1_c_CleanedByEvalue_ABScript_noLengthFilter/pbra.cleanedByEvalue.noLengthFilter.Pruned.all.pullOut.info.txt",header=T,sep="\t")
...
################# NON FUNCTIONAL ###############
################## a. TRUNCATED ###############
truncORs_NC <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/pbra/step_4_b_visualization/VisualizationNotes/pbra.Nmissing.Cmissing.IDList.format.txt",header=T,strip.white=T) # this is list of truncated ORs. 
dim(truncORs_NC) #77
truncORs_N <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/pbra/step_4_b_visualization/VisualizationNotes/pbra.Nmissing.Ccomplete.IDList.format.txt",header=T,strip.white=T)
dim(truncORs_N) # 64
truncORs_C <- read.table("/Users/annabelbeichman/Documents/UCLA/Otters/Olfaction/OR_PseudogeneResults/pbra/step_4_b_visualization/VisualizationNotes/pbra.NComplete.Cmissing.IDList.format.txt",header=T,strip.white=T)
dim(truncORs_C) # 45
...
dim(pgenes) # 670 
dim(allTrunc) # 185
length(unique(functionalORs_names)) # 585
# 670+185+585 = 1440
...
#pullOut_all[pullOut_all$bedname %in% exclude,]$label <- "exclude"
...
# check:
sum(pullOut_allPlusMissingFunc$label=="pseudogene") # 231 (670 with frags) 
sum(pullOut_allPlusMissingFunc$label=="functional") # 585
sum(pullOut_allPlusMissingFunc$label=="truncated") # 185 
sum(pullOut_allPlusMissingFunc$label=="fragment") # 439 
...
######## write out:
write.table(pullOut_allPlusMissingFunc,"step_5_labelAllgenes/pullOut.func.trunc.pgene.labelled.pbra.all.AddedMissing.fragments.txt",row.names=F,quote = F,sep="\t")
...
p2 <- ggplot(pullOut_allPlusMissingFunc,aes(x=clade,fill=label))+
  ...
  ggtitle("Giant Otter Olfactory Receptors")+
