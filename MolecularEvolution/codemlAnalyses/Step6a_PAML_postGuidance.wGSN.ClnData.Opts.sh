...
#$ -e pamlReports
#$ -t 1-77
### this is for running paml post GUIDANCE and optionally after gblocks or swamp
display_usage() { 
	echo "This script requires you to specify: 1) species for the foreground branch (otters, elut, pbra) \n 2) rundate for this run of paml (YYYYMMDD) \n 3) G/S/N (gblocks, swamp, none) 4) (optional) if S, need to specify swamp date \n Note that date you aligned orthologous clusters is defined below -- if re-aligned, change it." 
	echo -e "\nUsage: qsub script.sh [species] [date] [filter: G/GMED/S/N (gblocks (original, bad block size),gblocks medium (with block size of 30, gb-med suffix), swamp or none)] [Cleandata 0 or 1] OPTIONAL if S: swamp run date \n" 
	}
	
# if fewer than four arguments supplied, display usage 
	if [  $# -lt 4 ] 
	then 
		display_usage
		exit 1
...
rep=1 # if you're running paml multiple times for convergence  
...
genericAlt=$wd/paml_generic_files/generic.codeml.alt.CleanData${clnData}.ctl
#### set up 
...
then
echo
"filter was Gblocks-medium (block size 30)"
